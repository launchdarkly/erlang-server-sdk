...
end_per_testcase(_, _Config) ->
    ...
    ok = ldclient_storage_ets:empty(another1, features),
    ok.
...
server_init(_) ->
    ...
    {error, already_exists, _} = ldclient_storage_ets:create(default, features),
    {error, already_exists, _} = ldclient_storage_ets:create(default, segments),
    {error, already_exists, _} = ldclient_storage_ets:create(another1, features),
...
server_get_upsert(_) ->
    [] = ldclient_storage_ets:get(default, features, <<"flag1">>),
    ok = ldclient_storage_ets:upsert(default, features, #{<<"flag1">> => [{<<"value1">>, 0.5}]}),
    [{<<"flag1">>, [{<<"value1">>, 0.5}]}] = ldclient_storage_ets:get(default, features, <<"flag1">>),
    [] = ldclient_storage_ets:get(another1, features, <<"flag1">>),
    ok = ldclient_storage_ets:upsert(another1, features, #{<<"flag1">> => [{<<"valueA">>, 0.9}]}),
    [{<<"flag1">>, [{<<"valueA">>, 0.9}]}] = ldclient_storage_ets:get(another1, features, <<"flag1">>),
    [{<<"flag1">>, [{<<"value1">>, 0.5}]}] = ldclient_storage_ets:get(default, features, <<"flag1">>).

server_upsert_clean(_) ->
    ok = ldclient_storage_ets:upsert(default, features, #{<<"flag1">> => [{<<"value1">>, 0.5}]}),
    ok = ldclient_storage_ets:upsert_clean(default, features, #{<<"flag2">> => [{<<"value2">>, 0.9}]}),
    [{<<"flag2">>, [{<<"value2">>, 0.9}]}] = ldclient_storage_ets:all(default, features).

server_all(_) ->
    [] = ldclient_storage_ets:all(default, features),
    [] = ldclient_storage_ets:all(another1, features),
    ok = ldclient_storage_ets:upsert(default, features, #{<<"flag1">> => [{<<"value1">>, 0.5}], <<"flag2">> => [{<<"value2">>, 0.7}]}),
    ok = ldclient_storage_ets:upsert(another1, features, #{<<"flag1">> => [{<<"value1">>, 0.9}], <<"flag5">> => [{<<"value2">>, 0.77}]}),
    [
        {<<"flag1">>, [{<<"value1">>, 0.5}]},
        {<<"flag2">>, [{<<"value2">>, 0.7}]}
    ] = lists:sort(ldclient_storage_ets:all(default, features)),
    [
        {<<"flag1">>, [{<<"value1">>, 0.9}]},
        {<<"flag5">>, [{<<"value2">>, 0.77}]}
    ] = lists:sort(ldclient_storage_ets:all(another1, features)).
...
server_process_events_put(_) ->
    ...
    [
        ...
        {<<"segment-key-2">>, #{deleted := false,excluded := [],included := [],key := <<"segment-value-2">>,rules := [],
              salt := <<>>,version := 0}}
    ] = lists:sort(ldclient_storage_ets:all(default, segments)).
