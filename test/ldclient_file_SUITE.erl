...
check_file_json(_) ->
    {{1, false, off}, _} = ldclient_eval:flag_key_for_user(default, <<"keep-it-off">>, #{key => <<"user123">>}, "foo"),
...
check_file_no_segments_json(_) ->
    {{0, true, fallthrough}, _Events} = ldclient_eval:flag_key_for_user(no_segments, <<"keep-it-on">>, #{key => <<"user123">>}, "foo"),
    %% The segment isn't there, so the user isn't included.
    ExpectedReason = {rule_match, 1, <<"489a185d-caaf-4db9-b192-e09e927d070c">>},
    {{1, false, ExpectedReason}, _} =
        ldclient_eval:flag_key_for_user(no_segments, <<"segment-me">>, #{key => <<"user-12345">>}, "foo").

check_file_no_flags_json(_) ->
    {{null, "foo", {error, flag_not_found}}, _} = ldclient_eval:flag_key_for_user(no_flags, <<"keep-it-on">>, #{key => <<"user123">>}, "foo").

check_file_simple_flags_json(_) ->
    {{0, <<"value-1">>, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags, <<"my-string-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, true, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags, <<"my-boolean-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, 3, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags, <<"my-integer-flag-key">>, #{key => <<"user123">>}, "foo").

check_file_simple_flags_yaml(_) ->
    {{0, <<"value-1">>, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags_yaml, <<"my-string-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, true, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags_yaml, <<"my-boolean-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, 3, fallthrough}, _} = ldclient_eval:flag_key_for_user(simple_flags_yaml, <<"my-integer-flag-key">>, #{key => <<"user123">>}, "foo").

check_file_all_properties_yaml(_) ->
    {{2, <<"on">>, fallthrough}, _} = ldclient_eval:flag_key_for_user(all_properties_yaml, <<"flag1">>, #{key => <<"user123">>}, "foo"),
    {{0, <<"value-1">>, fallthrough}, _} = ldclient_eval:flag_key_for_user(all_properties_yaml, <<"my-string-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, true, fallthrough}, _} = ldclient_eval:flag_key_for_user(all_properties_yaml, <<"my-boolean-flag-key">>, #{key => <<"user123">>}, "foo"),
    {{0, 3, fallthrough}, _} = ldclient_eval:flag_key_for_user(all_properties_yaml, <<"my-integer-flag-key">>, #{key => <<"user123">>}, "foo").
...
check_with_missing_file(_) ->
    ...
    {{null, "foo", {error, client_not_ready}}, _} = ldclient_eval:flag_key_for_user(missing_and_valid_files, <<"my-string-flag-key">>, #{key => <<"user123">>}, "foo").
check_with_only_missing_file(_) ->
    {{null, "foo", {error, client_not_ready}}, _} = ldclient_eval:flag_key_for_user(missing_file, <<"keep-it-on">>, #{key => <<"user123">>}, "foo").

check_with_duplicate_flags(_) ->
    % With duplicate flags, and not allowing duplicate flags, the store will not be initialized.
    {{null, "foo", {error, client_not_ready}}, _} = ldclient_eval:flag_key_for_user(duplicate_flags, <<"keep-it-on">>, #{key => <<"user123">>}, "foo").
