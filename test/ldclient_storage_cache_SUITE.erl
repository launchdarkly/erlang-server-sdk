...
server_get_upsert(_) ->
    ...
    [{<<"flag1">>, #{debug_events_until_date := null,deleted := false,
               fallthrough := #{bucket_by := key,variations := []},
               key := <<"flag1">>,off_variation := 0,on := true,
               prerequisites := [],rules := [],salt := <<>>,targets := [],
               track_events := false,track_events_fallthrough := false,
    ...
    [] = ldclient_storage_redis:get(another1, features, <<"flag1">>),
    ok = ldclient_storage_redis:upsert(another1, features, #{<<"flag1">> => #{debug_events_until_date => null,deleted => false,
               fallthrough => #{bucket_by => key,variations => []},
               key => <<"flag1">>,off_variation => 0,on => false,
               prerequisites => [],rules => [],salt => <<>>,targets => [],
               track_events => false,track_events_fallthrough => false,
...
server_upsert_clean(_) ->
    ...
    [{<<"flag1">>, #{debug_events_until_date := null,deleted := false,
               ...
               track_events := false,track_events_fallthrough := false,
               variations := [],version := 0}}] = ldclient_storage_redis:get(default, features, <<"flag1">>),
    ok = ldclient_storage_redis:upsert_clean(default, features, #{<<"flag2">> => #{debug_events_until_date => null,deleted => false,
               fallthrough => #{bucket_by => key,variations => []},
               key => <<"flag2">>,off_variation => 0,on => false,
               prerequisites => [],rules => [],salt => <<>>,targets => [],
               track_events => false,track_events_fallthrough => false,
               variations => [],version => 1}}),
    [{<<"flag2">>, #{debug_events_until_date := null,deleted := false,
               fallthrough := #{bucket_by := key,variations := []},
               key := <<"flag2">>,off_variation := 0,on := false,
               prerequisites := [],rules := [],salt := <<>>,targets := [],
               track_events := false,track_events_fallthrough := false,
...
server_all(_) ->
    ...
    ok = ldclient_storage_redis:upsert(default, features, #{<<"flag1">> => #{debug_events_until_date => null,deleted => false,
               ...
               track_events => false,track_events_fallthrough => false,
               variations => [],version => 0}}),
    ok = ldclient_storage_redis:upsert(default, features, #{<<"flag2">> => #{debug_events_until_date => null,deleted => false,
               fallthrough => #{bucket_by => key,variations => []},
               key => <<"flag2">>,off_variation => 0,on => true,
               prerequisites => [],rules => [],salt => <<>>,targets => [],
               track_events => false,track_events_fallthrough => false,
               variations => [],version => 0}}),
    ok = ldclient_storage_redis:upsert(another1, features, #{<<"flag1">> => #{debug_events_until_date => null,deleted => false,
               ...
               track_events => false,track_events_fallthrough => false,
               variations => [],version => 1}}),
    ok = ldclient_storage_redis:upsert(another1, features, #{<<"flag5">> => #{debug_events_until_date => null,deleted => false,
               fallthrough => #{bucket_by => key,variations => []},
               key => <<"flag5">>,off_variation => 0,on => true,
    ...
    [
        {<<"flag1">>, #{debug_events_until_date := null,deleted := false,
               ...
               track_events := false,track_events_fallthrough := false,
               variations := [],version := 1}},
        {<<"flag5">>, #{debug_events_until_date := null,deleted := false,
               fallthrough := #{bucket_by := key,variations := []},
               key := <<"flag5">>,off_variation := 0,on := true,
               prerequisites := [],rules := [],salt := <<"launchdarkly">>,targets := [],
               track_events := false,track_events_fallthrough := false,
