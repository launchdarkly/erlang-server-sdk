...
init(_) ->
    StorageSup = ?CHILD(eld_storage_ets_sup, eld_storage_ets_sup, [], supervisor),
...
create(Bucket) ->
    eld_storage_ets_server:create(Bucket).

-spec empty(Bucket :: atom()) ->
    ok |
    {error, bucket_not_found, string()}.
empty(Bucket) ->
    eld_storage_ets_server:empty(Bucket).

-spec get(Bucket :: atom(), Key :: binary()) ->
    [{Key :: binary(), Value :: any()}] |
    {error, bucket_not_found, string()}.
get(Bucket, Key) ->
    eld_storage_ets_server:get(Bucket, Key).

-spec list(Bucket :: atom()) ->
    [{Key :: binary(), Value :: any()}] |
    {error, bucket_not_found, string()}.
list(Bucket) ->
    eld_storage_ets_server:list(Bucket).

-spec put(Bucket :: atom(), Items :: #{Key :: binary() => Value :: any()}) ->
    ok |
    {error, bucket_not_found, string()}.
put(Bucket, Items) ->
    eld_storage_ets_server:put(Bucket, Items).

-spec terminate() -> ok.
terminate() ->
    supervisor:terminate_child(eld_storage_ets_sup, eld_storage_ets_server).
